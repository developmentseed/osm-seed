...
if [ "$REPLICATION_URL" != "" ] && [ "$FREEZE" != "true" ]; then
  ...
  sudo -u nominatim nominatim replication --project-dir ${PROJECT_DIR} --init --verbose
  if [ "$UPDATE_MODE" == "continuous" ]; then
    echo "starting continuous replication"
    sudo -u nominatim nominatim replication --project-dir ${PROJECT_DIR} --check-for-updates --verbose &> /var/log/replication.log &
    replicationpid=${!}
  elif [ "$UPDATE_MODE" == "once" ]; then
    echo "starting replication once"
    sudo -u nominatim nominatim replication --project-dir ${PROJECT_DIR} --once --verbose &> /var/log/replication.log &
    replicationpid=${!}
  elif [ "$UPDATE_MODE" == "catch-up" ]; then
    echo "starting replication once in catch-up mode"
    sudo -u nominatim nominatim replication --project-dir ${PROJECT_DIR} --catch-up --verbose &> /var/log/replication.log &
    replicationpid=${!}
  else
